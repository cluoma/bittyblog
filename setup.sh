...
sqlite3 bittyblog.db "CREATE TABLE \`posts\` (\`id\` INTEGER NOT NULL PRIMARY KEY UNIQUE, \`page_id\` INTEGER, \`title\` TEXT, \`text\` TEXT, \`time\` INTEGER NOT NULL, \`extra\` TEXT, \`byline\` TEXT, \`thumbnail\` TEXT, \`visible\` INTEGER, FOREIGN KEY(\`page_id\`) REFERENCES pages ( id ));"
...
sqlite3 bittyblog.db "CREATE TABLE \`tags_pages_relate\` (\`tag_id\` INTEGER NOT NULL, \`page_id\` INTEGER NOT NULL, PRIMARY KEY(tag_id,post_id));"
